...
namespace Vanguard.Windows.InstanceManager
{
    public class ProcessLauncher
    {
        ...
        {
            public uint cb;
            [MarshalAs(UnmanagedType.LPTStr)] public string Reserved;
            [MarshalAs(UnmanagedType.LPTStr)] public string Desktop;
        ...
        [DllImport("advapi32.dll", SetLastError = true)]
        static extern bool SetSecurityDescriptorDacl(ref SECURITY_DESCRIPTOR sd, bool daclPresent, IntPtr dacl,
            bool daclDefaulted);
        ...
        public static Process RunProcess(string domain, string userName, string password, string workingDirectory, string applicationName, string arguments)
        {
            var token = IntPtr.Zero;
            var primaryToken = IntPtr.Zero;
            try
            {
                var result = LogonUser(userName, domain, password, (int)LOGON_TYPE.LOGON32_LOGON_INTERACTIVE, (int)LOGON_PROVIDER.LOGON32_PROVIDER_DEFAULT, out token);
                if (!result)
                {
                    var winError = Marshal.GetLastWin32Error();
                    Console.WriteLine("Failed to log in the service user: " + winError);
                }
                ...
                var ptr = Marshal.AllocCoTaskMem(Marshal.SizeOf(sd));
                ...
                {
                    var winError = Marshal.GetLastWin32Error();
                    Console.WriteLine("Failed to set the security descriptor: " + winError);
                }
                ...
                {
                    var winError = Marshal.GetLastWin32Error();
                    Console.WriteLine("Failed to duplicate the user token for impersonation: " + winError);
                }
                ...
                var startupInfo = new STARTUPINFO { Desktop = "" };

                result = CreateProcessAsUser(primaryToken, null, $"{Path.Combine(workingDirectory, applicationName)} {arguments}", ref processAttributes, ref threadAttributes, true, (uint)CreationFlags.CREATE_NO_WINDOW, environment, workingDirectory, ref startupInfo, out var processInfo);
                if (!result)
                {
                    var winError = Marshal.GetLastWin32Error();
                    Console.WriteLine("Failed to create process: " + winError);
                }
            ...
            {
                var winError = Marshal.GetLastWin32Error();
                Console.WriteLine("General error: " + winError);
            }
            finally
            {
                if (token != IntPtr.Zero)
                {
                    var x = CloseHandle(token);
                    if (x == 0)
